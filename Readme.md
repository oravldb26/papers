# Oracle at VLDB 2026

Explore Oracle’s latest research and technical papers presented at VLDB.

## Papers

### [Real-time SQL Plan Management in Oracle](PASTE_PAPER_URL_HERE)

Brief description of the paper.

### [Trusted Agentic AI with Oracle AI Database](PASTE_PAPER_URL_HERE)

Brief description of the paper.

### [Storage, Indexing & I/O Governance](PASTE_PAPER_URL_HERE)

Brief description of the paper.

### [Disaggregated Memory, Shared Storage & Cloud DB Architecture](PASTE_PAPER_URL_HERE)

Brief description of the paper.

---

For more information, visit [Oracle Database](https://www.oracle.com/database/).
